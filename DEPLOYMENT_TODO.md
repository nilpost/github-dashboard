...
- [ ] **Cloudflare API token** — scoped to **Zone → DNS → Edit** on
      `postiusgroup.com` (a Zone.DNS:Edit token is enough for the record; SSL-mode
      changes need Zone Settings edit or the dashboard)
...
4. [ ] **Railway:**
       ...
       railway up
       railway domain          # note the *.up.railway.app URL = the ORIGIN
       ```
5. [ ] **Confirm origin is up:** `node scripts/health-check.cjs https://<app>.up.railway.app`
       → `{"status":"ok"}`. This URL is what DNS points at.
...
3. [ ] Browser: register a user → add the GitHub token in Settings → run Sync;
       confirm login persists across a refresh (validates secure cookies).
